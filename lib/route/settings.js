...
const getScheduleProps = require('../util/getScheduleProps');
...
router.get('/general/', async (req, res) => {
  ...
  const companyDateFormats = company.get_available_date_formats();
  const shareAllAbsences = company.share_all_absences
  const isTeamViewHidden = company.is_team_view_hidden
  const scheduleMeta = getScheduleProps(schedule);
  ...
  const defaultResponse = () => res.render('general_settings', context);
  responseParser({
    req,
    res,
    defaultResponse,
...
router.post('/company/', function(req, res){
  ...
  .then(function(){
      ...
      const defaultResponse = () => res.redirect_with_session('/settings/general/');
      return responseParser({
        req,
        res,
        defaultResponse,
        statusCode: 200,
        body: { messages: ['Company was successfully updated'] }
      });
  })
...
router.post('/carryOverUnusedAllowance/', (req, res) => {
  let body = { message: 'Unused allowance was successfully carried over' };
  let statusCode = 200;
  req.user
    .getCompany()
    ...
    .catch(error => {
      ...
      body = {
        errors: ['Failed to carry over unused allowances'],
      };
      statusCode = 500;
    })
    .finally(() => {
...
router.post('/schedule/', function(req, res){
  let body = {
    messages: ['Schedule saved successfully'],
    isUserSpecific: true,
  };
  ...
  req.user.getCompany()
    ...
    .finally(function(){
      const defaultResponse = () => res.redirect_with_session(schedule.is_user_specific()
  ...
  req.user
    ...
    .then((leaveTypes) => {
      ...
      return responseParser({
        ...
        statusCode: 200,
        body: {
          messages: ['Changes to leave types were saved'],
          leaveTypes,
        },
      });
    })
...
router.post('/leavetypes/delete/:leave_type_id/', function(req, res){
    ...
    .then(function(leaveType){
        ...
        return responseParser({
          ...
          statusCode: 200,
          body: {
            messages: ['Leave type was successfully removed'],
            leaveType,
          }
        })
    })
...
router.get('/company/integration-api/', (req, res) => {
  req.user
    ...
    .then(company => {
      ...
      const defaultResponse = () => res.render('settings_company_integration_api', context)
      responseParser({
        req,
        res,
        defaultResponse,
...
router.post('/company/integration-api/', (req, res) => {
  ...
  action = action.then(() => {
    ...
    return responseParser({
      ...
      statusCode: 200,
      defaultResponse,
      body: { messages: ['Settings were saved'] },
    });
  });
...
router.get('/company/authentication/', function(req, res){
  ...
  req.user
    ...
    .then(function(company){
      ...
      const defaultResponse = () => res.render('settings_company_authentication', context);
...
router.post('/company/authentication/', function(req, res){
  ...
  req.user
    ...
    .then(function(){
      if ( req.session.flash_has_errors() ) {
        const defaultResponse = () => res.redirect_with_session('/settings/company/authentication/');
        return responseParser({
          req,
          res,
          defaultResponse,
