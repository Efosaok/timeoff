...
module.exports = function(passport) {
  ...
  router.get('/login', function(req, res){
      ...
      const render = () => res.render('login', context);
      responseParser({
        req,
        res,
  ...
  router.post('/login', function(req, res, next) {
    passport.authenticate('local', function(err, user) {
      if (err) { return responseParser({ next: () => next(err) }) }
      ...
      if (!user) {
        ...
        return responseParser({
          req,
          res,
          body: { errors: ['Incorrect credentials'] },
          defaultResponse: () => res.redirect_with_session('/login'),
          statusCode: 400,
        })
      }
      ...
      req.logIn(user, function(err) {
        ...
        return responseParser({
          ...
          body: {
            user,
            messages: [`Welcome back ${user.name}!`]
          },
  ...
  router.get('/logout', function(req, res){
      ...
      if ( !req.user ) {
          const defaultResponse = () => res.redirect_with_session(303, '/');
          return responseParser({
            req,
            res,
            defaultResponse,
            statusCode: 400,
            body: { errors: ['Cannot logout']}
          })
      }

      req.logout();

      const defaultResponse = () => res.redirect_with_session(res.locals.url_to_the_site_root);
      return responseParser({
        req,
        res,
        defaultResponse,
  ...
  router.post('/forgot-password/', function(req, res){
    ...
    if ( req.session.flash_has_errors() ) {
      ...
      return responseParser({
        ...
        defaultResponse,
        statusCode: 400,
        body: { errors: req.session.flash.errors, redirect: true }
      });
    }
    ...
    req.app.get('db_model').User.get_user_by_reset_password_token(token)
      .then(function(user){
        if (! user) {
          ...
          const defaultResponse = () => res.redirect_with_session('/forgot-password/');
          return responseParser({
            req,
            res,
            defaultResponse,
        ...
        const defaultResponse = () => res.render('reset_password', context);
        responseParser({
          req,
          res,
          defaultResponse,
  ...
  router.post('/reset-password/', function(req, res){
    ...
    if (password !== confirm_password) {
      ...
      const defaultResponse = () => res.redirect_with_session('/reset-password/?t='+token);
      return responseParser({
        req,
        res,
        defaultResponse,
        statusCode: 400,
        body: { errors: req.session.flash.errors, token, }
      });
    }
    ...
    req.app.get('db_model').User.get_user_by_reset_password_token(token)
      ...
      .then(function(){
        ...
        const defaultResponse = () => res.redirect_with_session('/login/');
        return responseParser({
          req,
          res,
          defaultResponse,
          statusCode: 200,
          body: { message: 'Please use new password to login into system'  }
        })
      });
  });
