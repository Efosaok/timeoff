...
function get_and_validate_department(args) {
  ...
  if (!validator.isFloat(allowance)) {
    ...
    req.session.flash_error(
      'New allowance for '+department_name+' should be between 0.25 and 50 days'
    );
  }
...
router.get('/departments/', function(req, res){
  ...
  if (!req.headers.react_app) res.locals.custom_java_script.push('/js/departments.js');
  ...
  .then(function(departments){
    ...
    const defaultResponse = () => res.render('departments_overview', context);
    responseParser({
      req,
      res,
      defaultResponse,
...
router.post('/departments/', function(req, res){
  ...
  .then((department) => {
    ...
    const defaultResponse = () => res.redirect_with_session('/settings/departments/');
...
router.post('/departments/delete/:department_id/', function(req, res){
  ...
  .then(function(dpt){
    ...
    return responseParser({
      ...
      defaultResponse,
      statusCode: 200,
      body: { messages: ['Department was successfully removed'], dpt }
    })
  })
  .catch(function(error){
    ...
    const defaultResponse = () => res.redirect_with_session( department_to_remove
...
router.get('/departments/edit/:department_id/', function(req, res){
  ...
  .then(function(result){
    ...
    const defaultResponse = () => res.render('department_details', context);
    responseParser({
      req,
      res,
      defaultResponse,
  ...
  .catch(function(error){
    ...
    return responseParser({
      ...
      defaultResponse,
      statusCode: 500,
      body: { errors: ['Failed to fetch details for given department'] }
    });
  });
});
...
router.post('/departments/edit/:department_id/', function(req, res){
  ...
  .then(function(){
    if (req.body.remove_supervisor_id) {
      ...
      .then(function(dpt){
        const message = `Supervisor was removed from ${department.name}`;
        body.messages = [message];
        req.session.flash_message(message);
        return Promise.resolve(dpt);
      });
      ...
      .then(function(dpt){
        const message = `Supervisors were added to department ${department.name}`;
        body.messages = [message];
        req.session.flash_message(message);
        return Promise.resolve(dpt);
      });
    ...
    .then(function(dpt){
      const message = `Department ${department.name} was updated`;
      body.messages = [message];
