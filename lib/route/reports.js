...
router.get('/allowancebytime/', (req, res) => {
  ...
  Promise.join(
    ...
    .catch(error => {
      ...
      const defaultResponse = () => res.redirect_with_session(redirect_path);
...
function render_allowancebytime(args) {
  let
    ...
    const context = {
      users_and_leaves    : JSON.parse(convertCircular(team_view_details.users_and_leaves)),
      related_departments : JSON.parse(convertCircular(team_view_details.related_departments)),
    ...
    const defaultResponse = () => res.render('report/allowancebytime', context);
...
router.get('/leaves/', async (req, res) => {
  ...
  try {
    ...
    const defaultResponse = () => res.redirect_with_session(redirectPath);
    return responseParser({
      req,
      res,
      defaultResponse,
  ...
  if (renderAsCsv) {
    ...
    const defaultResponse = () => res.render('report/leaves', context);
    responseParser({
      req,
      res,
      defaultResponse,
