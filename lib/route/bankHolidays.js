...
router.get('/bankholidays/', (req, res) => {
  ...
  if (!req.headers.react_app) res.locals.custom_java_script.push(
    '/js/bank_holidays.js'
  );
  ...
  .then((company) => {
    ...
    const context = {
      company,
      calendar: convertCalendarSundayToSaturday(calendar),
      bankHolidays,
    ...
    const defaultResponse = () => res.render('bankHolidays', context);
    responseParser({
      req,
      res,
      defaultResponse,
...
router.post('/bankholidays/', function(req,res){
  ...
  .then((bankHolidays) => {
    let statusCode = 200;
    let body = {
      messages: ['Changes to bank holidays were saved'],
      bankHolidays: bankHolidays?.[0],
    };
    ...
    const defaultResponse = () => res.redirect_with_session(`/settings/bankholidays/?year=${currentYear}`);
...
router.post('/bankholidays/delete/:bankHolidayId/', function(req, res){
  ...
  .then((bankHoliday) => {
    ...
    return responseParser({
      ...
      statusCode: 200,
      body: {
        messages: ['Bank holiday was successfully removed'],
        bankHoliday,
      }
    })
  });
