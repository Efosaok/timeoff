...
var express   = require('express'),
    ...
    responseParse = require('../util/responseParser'),
...
router.get('/', function(req, res){
  ...
  Promise.join(
      ...
      function(my_leaves_grouped, to_be_approved_leaves){
        const context = {
          groupedLeavesMeta: generateGroupedLeavesMeta(my_leaves_grouped, to_be_approved_leaves),
          my_leaves_grouped,
          to_be_approved_leaves,
        };
        const defaultResponse = () => res.render('requests', context);
        responseParse({
          req,
          res,
          defaultResponse,
...
function leave_request_action(args) {
    ...
    const approver_comment = validator.trim( req.body['approver_comment'] );
    const rejecter_comment = validator.trim( req.body['rejecter_comment'] );
    const general_comments = validator.trim( req.body['general_comments'] );
    const mod_leave_type = validator.trim(req.body['mod_leave_type']);
    const meta = { approver_comment, rejecter_comment, general_comments, mod_leave_type };
    ...
    .then(function(leaves){
       ...
       return leave_to_process[leave_action_method]({ by_user : req.user, meta });
    })
    ...
    .then(function(processed_leave){
      ...
      return responseParser({
        ...
        statusCode: 200,
        body: {
          messages: ['Request from '+processed_leave.user.full_name()
          +' was processed'],
          leave: processed_leave,
        },
      });
...
router.post('/cancel/', function(req, res){
  ...
  .finally(function(){
    let statusCode = 200;
    let body = { messages: ['The leave request was canceled'] }
    if (req.session.flash_has_errors()) {
      statusCode = 500;
      body = { errors: req.session.flash.errors }
    }
    const defaultResponse = () => res.redirect_with_session('/requests/');
    return responseParser({
      req,
      res,
      defaultResponse,
    ...
    Promise
      ...
      .then(processed_leave => {
        req.session.flash_message(
          'You have requested leave to be revoked. check your mail box for confirmation of approval'
        );
