...
const
  ...
  responseParser = require('../../util/responseParser'),
  getScheduleProps = require('../../util/getScheduleProps'),
...
router.get('/add/', function(req, res){
  req.user
    ...
    .then(function(company){
      ...
      const defaultResponse = () => res.render('user_add', context);
      responseParser({
        req,
        res,
        defaultResponse,
...
router.post('/add/', function(req, res){
  ...
  .then(function(new_user){
    if ( req.session.flash_has_errors() ) {
      const defaultResponse = () => res.redirect_with_session('../add/');
      ...
      return responseParser({
        ...
        defaultResponse,
        statusCode: 200,
        body: { message: ['New user account successfully added'], new_user }
      });
    }
  })
...
router.get('/import/', function(req, res){
  req.user
    ...
    .then(company => {
      ...
      const defaultResponse = () => res.render('users_import', context);
      responseParser({
        req,
        res,
        defaultResponse,
...
// console.error('I got here 11');
  Promise
  .try(() => ensure_user_id_is_integer({req : req, user_id : user_id}))
//  .then(() => console.log('I got here 11.1'))
  .then(() => req.user.get_company_for_user_details({user_id}))
//  .then(() => console.log('I got here 11.2'))
  .then(function(company){

// console.error('I got here 4');
    const employee = company.users[0];
// console.error('I got here 5');

    return employee.promise_schedule_I_obey()
      .then(function(){
        const employeeMeta = {};
        employeeMeta.isActive = employee.is_active();
        const context = {
          employeeMeta,
          company,
          employee,
        ...
        const defaultResponse = () => res.render('user_details', context);
        responseParser({
          req,
          res,
          defaultResponse,
  ...
  .catch(function(error){
    ...
    return responseParser({
      ...
      defaultResponse,
      statusCode: 500,
      body: { errors: ['Unable to fetch employee details'] }
    });
  });
});
      ...
      .then(function(){
        employee
          ...
          .then(function(grouped_leaves){
            ...
            const context = {
              groupedLeavesMeta: generateGroupedLeavesMeta(grouped_leaves),
...
router.get('/edit/:user_id/schedule/', function(req, res){
    ...
    return employee
      ...
      .then(function(schedule){
        const context = {
          employee,
          schedule,
          scheduleMeta: getScheduleProps(schedule),
          isUserSpecific: schedule.is_user_specific(),
  ...
  .catch(function(error){
    ...
    return responseParser({
      ...
      defaultResponse,
      statusCode: 500,
      body: { errors: ['An error occured trying to get employee absence']}
    });
  });
});
...
router.get('/edit/:user_id/calendar/', async (req, res) => {
  ...
  try {
    ...
    return responseParser({
      ...
      defaultResponse,
      statusCode: 500,
      body: { errors: ['An error occured trying to render calendar']}
    });
  }
  ...
  const context = {
    ...
    allowanceMeta: getUserAllowanceMeta(userAllowance),
    calendar : convertCalendarSundayToSaturday(calendar.map(c => c.as_for_template())),
  ...
  .then(function(){
    ...
    employee
      ...
      .then(function(){
        ...
        const defaultResponse = () => res.redirect_with_session(req.body.back_to_absences ? './absences/' : '.');
        return responseParser({
          req,
          res,
          defaultResponse,
          statusCode: 200,
          body: { messages: [req.session.flash.messages] },
        });
      });
  })
...
router.post('/delete/:user_id/', function(req, res){
    ...
    .then(() => {
      ...
      return responseParser({
        ...
        statusCode: 200,
        body: {
          messages: ['Employee records were removed from the system'],
    ...
    .catch(error => {
      ...
      const defaultResponse = () => res.redirect_with_session(`../../edit/${user_id}/`);
      return responseParser({
        req,
        res,
        defaultResponse,
...
router.get('/', function(req, res) {
    ...
    .then(function(args){
      ...
      const defaultResponse = () => res.render('users', renderContext);
      responseParser({
        req,
        res,
        defaultResponse,
...
function get_and_validate_user_parameters(args) {
    ...
    } else if (adjustment && ! ( adjustment % 1 === 0 || Math.abs( adjustment % 1 ) === 0.25 || Math.abs( adjustment %1 ) === 0.50 || Math.abs( adjustment %1 ) === 0.75)) {
      req.session.flash_error(
