...
const timeToDaysDeductedMap = require('../../util/timeToDaysDeductedMap');
...
// Determine if leave starts with quarter day in the morning
//
does_start_quarter_morning : function() {
  return this.day_part_start === Leave.leave_day_part_quarter();
},

does_start_quarter_afternoon : function() {
  return this.day_part_start === Leave.leave_day_part_quarter();
},
	//
...
// Determine if leave ends with quarter a day in the afternoon
//
does_end_quarter_afternoon : function() {
  return this.day_part_end === Leave.leave_day_part_quarter();
},

does_end_quarter_morning : function() {
  return this.day_part_end === Leave.leave_day_part_quarter();
},
...
get_deducted_days_number : function(args) {
  ...
  if (number_of_days === 1 && this.time) {
    number_of_days = timeToDaysDeductedMap[this.time];
  ...
  if (number_of_days === 1 && !this.get_start_leave_day().is_all_day_leave()) {
	  if ((this.day_part_start === Leave.leave_day_part_quarter() || this.day_part_end === Leave.leave_day_part_quarter())) {
		  number_of_days = number_of_days - 0.75;
	  } else {
  ...
  // are halfs
    ...
    if ( ! this.get_start_leave_day().is_all_day_leave() ){
	    if (this.day_part_start === Leave.leave_day_part_quarter()) {
		    number_of_days = number_of_days - 0.75;
	    } else {
      number_of_days = number_of_days - 0.5;
	    }
    ...
    if ( ! this.get_end_leave_day().is_all_day_leave() ) {
	    if (this.day_part_end === Leave.leave_day_part_quarter()) {
		    number_of_days = number_of_days - 0.75;
	    } else {
      number_of_days = number_of_days - 0.5;
    }
...
promise_to_reject : function(args) {
  ...
  self.rejecter_comment = args.meta.rejecter_comment || self.rejecter_comment;
  self.leaveTypeId = args.meta.mod_leave_type || self.leaveTypeId;
...
promise_to_approve : function(args) {
  ...
  self.approverId = by_user.id;
  self.approver_comment = args.meta.approver_comment || self.approver_comment;
  self.leaveTypeId = args.meta.mod_leave_type || self.leaveTypeId;
...
promise_to_revoke : function(){
  ...
  return self.getUser({
    ...
    .then(function(user){
      ...
      var new_leave_status = Leave.status_rejected();
