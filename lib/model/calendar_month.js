...
CalendarMonth.prototype.is_leave_quarter = function(day) {
    var leave_day = this._leaves[
        this.get_base_date().date(day).format(this.default_date_format())
    ];

    return this.is_leave(day)
        || (leave_day && leave_day.is_quarter_leave());
};
...
CalendarMonth.prototype.get_quarter_leave_type_id = function(day){
  let
    self = this,
    leave_day = self._leaves[
      self.get_base_date().date(day).format(self.default_date_format())
  ...
  return leave_day.get_quarter_leave_type_id();
};
    ...
    for( var i=1; i<=self.how_many_days(); i++){
        ...
        if ( self.is_weekend(i) ) {
            ...
            if ( self.is_leave_morning(i) ) {
              ...
              leaveLabelForDay.morning = self._leave_types_map[ morning_leave_type_id ].name || 'AM 1/2 Day';
            }
            ...
            if ( self.is_leave_afternoon(i) ) {
              ...
              leaveLabelForDay.afternoon = self._leave_types_map[ afternoon_leave_type_id ].name || 'PM 1/2 Day';
            }

            if ( self.is_leave_quarter(i) ) {
              day.is_leave_quarter= true;

              let quarter_leave_type_id = self.get_quarter_leave_type_id(i);
              day.quarter_leave_type_id = quarter_leave_type_id;

              day.leave_color_class_quarter = quarter_leave_type_id && self._leave_types_map[ quarter_leave_type_id ]
                ? self._leave_types_map[ quarter_leave_type_id ].get_color_class()
                : 'leave_type_color_1';

              leaveLabelForDay.quarter = 'Quarter Day';
            }
            ...
            if (leaveLabelForDay.morning && (leaveLabelForDay.morning === leaveLabelForDay.afternoon)) {
              ...
              if (leaveLabelForDay.morning) {
                day.leaveLabel = `${leaveLabelForDay.morning} (AM 1/2 Day) `;
              }
              if (leaveLabelForDay.afternoon) {
                day.leaveLabel = `${day.leaveLabel ? day.leaveLabel : ''}${leaveLabelForDay.afternoon} (PM 1/2 Day)`;
              }
              if (leaveLabelForDay.quarter) {
                day.leaveLabel = `${day.leaveLabel ? day.leaveLabel : ''}${leaveLabelForDay.quarter} (Quarter Day)`;
              }
            }
        }
